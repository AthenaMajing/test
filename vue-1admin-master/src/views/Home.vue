...
<script>
	export default {
		...
		mounted() {
			...
			if(localStorage.url=='all'){
				
				for (var it of this.$router.options.routes){
					it['hidden'] = false
				} 
			}else{
				var urls =localStorage.url
				urls = urls.split(',')
				console.log(urls)
				for (var item of this.$router.options.routes){
					for (var url of urls){
						if (url==item.path){
							console.log(item.path,url,'这是if')
							item['hidden'] = false
							console.log(item['hidden'])
							
						}
						// else if(url!=item.path){
						// 	console.log(url,item.path,'这是else')
						// 	item['hidden'] = true
						// }
