<template>
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				...
				<el-form-item>
					...
					<el-button type="primary" v-on:click="addpermission">添加</el-button>
				</el-form-item>
			</el-form>
		</el-col>
		...
		<el-table :data="users" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="id" label="序号" width="150" sortable>
			</el-table-column>
			<el-table-column prop="permissionname" label="权限名称" width="180">
			</el-table-column>
			<el-table-column prop="url" label="url" width="1150">
			</el-table-column>
			...
			<!--<el-table-column label="操作" width="300">
				<template slot-scope="scope">
					<el-button size="small" @click="addpermission()">新增权限</el-button>
				</template>
			</el-table-column>-->
		</el-table>
		...
		<el-dialog title="修改信息" v-model="editFormVisible" :close-on-click-modal="false">
			<el-form :model="editForm" label-width="120px" :rules="formRules" ref="editForm">
				...
				<el-form-item label="url" prop="url">
					<el-input v-model="editForm.url" :disabled="false"> </el-input>
				</el-form-item>
				<el-form-item label="权限" prop="feature">
					<el-input v-model="editForm.feature" :disabled="false"> </el-input>
				</el-form-item>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getUsers() {
				...
				if(this.filters.name){
					var url = "showpermission/"+this.filters.name+'/'+ this.page +"/"
				}else{
					var url = "showpermission/alls/"+ this.page +"/"
				}
				...
				this.$http.get(url, {
					...
					.then(response => {
						...
						if (response.data.data instanceof Array){
							...
							this.users.push({
								"permissionname":i.feature,
								"url":i.url,
								"id":i.id,
								// "superuser":i.is_superuser,
								// "email":i.email,
								// "permission":i.user_permission
							})
						}
						}else{
							this.users.push({
								"permissionname":response.data['feature'],
			...
			handleEdit: function (index, row) {
				...
				this.editForm = {}
				let token = sessionStorage.token || localStorage.token 
				var url = "/permission/"+ row.id + "/"+ this.page +"/"
				console.log()
				this.$http.get(url,{
					headers: {
						'Authorization': 'JWT '+ token
				...
				.then(response => {
					this.listLoading = false;
					let superuser = ""
					let permission_info = response.data
					console.log(permission_info)
					// if (user_info.is_superuser == 0){
					// 	superuser = "客服"
					// }else if (user_info.is_superuser == 1){
					// 	superuser = "管理员"
					// }
					this.editForm = {
						permission_id : permission_info.id,
						feature : permission_info.feature,
						url : permission_info.url,
						// id : permission_info.work_id,
						// email : permission_info.email,
						// superuser : superuser,
						// permission : "",
						// desc : permission_info.desc
					}
					this.listLoading = false;
			...
			editSubmit: function () {
				// 判断信息缺失
				if (this.editForm.url == "" || this.editForm.feature == ""){
				...
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							console.log(para)
							// if (para.is_superuser == "客服"){
							// 	para.is_superuser = 0
							// }else if (user_info.is_superuser == "管理员"){
							// 	suser_info.is_superuser = 1
							// }
							let token = sessionStorage.token || localStorage.token;
							para.date_joined = util.formatDate.format(new Date(), 'yyyy-MM-dd hh:mm:ss');
							let url = "/permission/"+ this.editForm.permission_id + "/"
			...
			deleteEdit: function () {
				...
				this.$confirm('确认删除该名管理员的所有信息吗？', '提示', {}).then(()=>{
					this.listLoading = true;
					let token = sessionStorage.token || localStorage.token 
					var url = "/deleteuser/"+ this.editForm.user_id + "/"
					let para = {
						'is_deleted':1
					}
					this.$http.put(url, para,{
		...
		mounted() {
			console.log("begthis.editForm.id")
