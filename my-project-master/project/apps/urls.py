...
urlpatterns = [
    ...
    url(r'^roles/(?P<permission_id>\d+)/(?P<page>\d+)/$',views.UpdateRoleViewSet.as_view({'get': 'retrieverole'})),
    url(r'^roles/(?P<pk>\d+)/$',views.UpdateRoleViewSet.as_view({'post': 'updatesrole'})),
