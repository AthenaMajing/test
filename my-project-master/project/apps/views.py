...
from apps.serializers import CreateUserSerializer, CreatePermissionSerializer, UserShowSerializer, \
    ...
    UpdatePermissionInfoSerializer, UpdateRoleInfoSerializer
        ...
        if select_name=='all':
            ...
            for user in user_info:
                ...
                for user_role in user_roles:
                    ...
                    dict1['title'] = title
                    permissions = user_role.permission.all()
                    for permission in permissions:
                        feature = permission.feature
                        dict1['feature'] = feature
                list.append(dict1)
            ...
            dict={}
            user = User.objects.get(username=select_name)
            roles = user.role.all()
            list_title=[]
            list_permission = []
            for role in roles :
                ...
                for permission in permissions:
                    feature = permission.feature
                    list_permission.append(feature)

            dict['title'] = list_title
            dict['permission'] = list_permission
            dict['username'] = user.username
            dict['email'] = user.email
            dict['id'] = user.id
            dict['date_joined'] = user.date_joined
            # user_info_json = self.get_serializer(instance=user)
            print(dict)
            return Response(dict)
...
class RoleInfoShow(ListAPIView,GenericAPIView):
    ...
    def get(self, request, select_name ,page):
        if select_name=='all':
            ...
            permission_info = Role.objects.get(title=select_name)
            print(permission_info)
            permission_info_json = self.get_serializer(instance=permission_info)
...
class UpdateRoleViewSet(ModelViewSet):
    '''
    修改角色信息
    '''
    queryset = Role.objects.all()
    serializer_class = UpdateRoleInfoSerializer
    def updatesrole(self, request,pk, *args, **kwargs):
        print(self.update(self.request),'tfftyjuii')

        return self.update(self.request)


    def retrieverole(self, request, permission_id,*args, **kwargs):
        try:
            role = Role.objects.get(id=permission_id)
        except Role.DoesNotExist:
            return Response(status=status.HTTP_404_NOT_FOUND)
        serializer = UpdateRoleInfoSerializer(role)
        # print(serializer.data)
        return Response(serializer.data)
