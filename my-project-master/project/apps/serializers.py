...
class CreateUserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ('id', 'username', 'password','password2','role','email','date_joined')
        extra_kwargs = {
            'username': {
                'min_length': 5,
                'max_length': 20,
    ...
    def create(self, validated_data):
        ...
        # 创建并保存新用户
        user = User.objects.create_user(username=username,password=password,email=email,date_joined=date_joined)
        for role in roles:
            print(role)
            user.role.add(role)
...
class UserShowSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ('id', 'username','is_deleted','is_superuser','date_joined','email')
...
class UpdateRoleInfoSerializer(serializers.ModelSerializer):
    '''
    更新角色信息
    '''
